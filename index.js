...
// We want to transform the array into another array.
// The callback is a fruitful function that returns a modified version of the element.
...
// const liNames = names.map(function (name) {
//     return `<li>${name}</li>`
// })
...
// We want to keep only some of the elements of the array.
// The callback function is executed for each element of the array 
// and the resulting array is including only the elements returning true.
...
// We want to retrieve ONE element of the array.
// The callback function gets executed for every element and the first one returning true
// gets returned from .find()

const book = books.find(book => book.price < 10)

// Equivalent to:
// const book = books.filter(book => book.price < 10)[0]
...
// We want to retrieve ONE index of one element of the array.
// The callback function gets executed for every element and the index of the first element returning true
// gets returned from .findIndex()

const bookIndex = books.findIndex(book => book.price > 30)

console.log(books[bookIndex])

// ARRAY SOME
// NON mutating
// We want to know if there is at least one element of the array
// satisfying the condition in the callback function

const hasCheapBook = books.some(book => book.price < 0)
console.log({ hasCheapBook })


// ARRAY EVERY
// NON mutating

// We want to know if ALL the elements in the array
// satisfy the condition in the callback function
const hasOnlyCheapBook = books.every(book => book.price < 10)

// ARRAY REDUCE
// NON mutating

// We want to perform a transformation of the array 
// parsing one by one its elements and returning the final result.
...
//         return accumulator + currentValue // new accumulator
//     }
